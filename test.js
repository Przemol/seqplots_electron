...
describe('==>> Application launch', function () {
  ...
  it('SeqPlots tests', function () {
    ...
    .waitUntilTextExists('.popover-title', 'Bring up plot setup window', 120000).pause(500)
    .click('[data-target="#calcModal"]')
    .then(function () {console.log('\tCalc modal up')})
    .waitUntilTextExists('.popover-title', 'Select track', 120000).pause(500)
    .then(function() {
      console.log('\t - taking s-shot')
      app.browserWindow.capturePage().then(function (imageBuffer) {
    ...
    .waitUntilTextExists('.popover-title', 'Select genomic features', 120000).pause(500)
    .click('*=Features')
    .then(function () {console.log('\tFeatures tab')})
    .waitUntilTextExists('.popover-title', 'Select 1st set of genomic intervals for plotting', 120000).pause(500)
    .click('tr*=Genes_celegans_bottom_20pct_expression_chr1.bed')
    .then(function () {console.log('\tFeature 1 selected')})
    .waitUntilTextExists('.popover-title', 'Select 2nd set of genomic intervals for plotting', 120000).pause(500)
    .click('tr*=Genes_celegans_top_20pct_expression_chr1.bed')
    .then(function () {console.log('\tFeature 2 selected')})
    .waitUntilTextExists('.popover-title', 'Select motifs', 120000).pause(500)
    .click('[data-value="Sequence features"]')
    .waitUntilTextExists('.popover-title', 'Select motifs - pattern', 120000).pause(500)
    .click('#SFpattern')
    .keys('GC')
    .waitUntilTextExists('.popover-title', 'Select motifs - name', 120000).pause(500)
    .click('#SFname')
    .keys('CpG')
    .waitUntilTextExists('.popover-title', 'Add motif', 120000).pause(500)
    .click('#SFadd')
    .then(function () {console.log('\tSequence selected')})
    .waitUntilTextExists('.popover-title', "Select plot type", 120000).pause(500)
    .click('#plot_type')
    .waitUntilTextExists('.popover-title', "Start calculation", 120000).pause(500)
    .click('#runcalc')
    .then(function () {console.log('\tRun calc')})
    .waitUntilTextExists('.popover-title', "Select what to plot", 120000).pause(500)
    .click('input[value="[1,1]"]').pause(100)
    .waitUntilTextExists('.popover-title', "Select what to plot", 120000).pause(500)
    .click('input[value="[1,2]"]')
    .waitUntilTextExists('.popover-title', "Plot average signal profile", 120000).pause(500)
    .click('#replotL')
    .waitUntilTextExists('.popover-title', "Toggle column selection", 120000).pause(500)
    .then(function () {console.log('\t1st plot done')})
    .click('thead th:nth-child(2) button').pause(200)
    .waitUntilTextExists('.popover-title', "Toggle column selection", 120000).pause(500)
    .click('thead th:nth-child(3) button')
    .waitUntilTextExists('.popover-title', "Re-plot average signal profile", 120000).pause(500)
    .click('#replotL')
    .waitUntilTextExists('.popover-title', "Select none", 120000).pause(500)
    .then(function () {console.log('\t2nd plot done')})
    .click('thead th:nth-child(1) button:nth-child(3)')
    .waitUntilTextExists('.popover-title', "Toggle row selection", 120000).pause(500)
    .click('tbody tr:nth-child(2) a')
    .waitUntilTextExists('.popover-title', "Plot heatmap", 120000).pause(500)
    .click('#replotH')
    .waitUntilTextExists('.popover-title', "The plot preview", 120000).pause(500)
